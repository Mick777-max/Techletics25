...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!name || !price || !category || !branch || !eventType || !startDateStr || !endDateStr) {
      return NextResponse.json(
        { success: false, error: 'Required fields: name, price, category, branch, eventType, startDate, endDate' },
        { status: 400 }
      );
    }
    ...
    const allowedBranches = ["cs", "me", "ec", "ce", "bsh", "ds", "eee"] as const;
    const allowedTypes = ["competition", "workshop", "techtalk", "expo"] as const;
